 ...
 * @ClassName JSTableDefaultModel
 * @Description 默认的实体类型的数据操作对象
 * @Author YueLifeng
 * @Date 2019/4/19 0019下午 4:03
 * @Version 1.0
 */
public class JSTableDefaultModel<E> extends JSTableModel<Collection<E>> {
    ...
    //原始的值
    private volatile Collection<E> original;
    ...
    @Override
    public JSTableModelListener<Collection<E>> getTableModelListener() {
        if (super.getTableModelListener() == null)
            this.setTableModelListener(new JSTableModelDefaultAdapter<E>());
        return super.getTableModelListener();
    }

    @Override
    public Collection<E> getOriginal() {
        return this.original;
    }

    @Override
    public void setOriginal(Collection<E> original) {
    ...
    public void tableChanged(TableModelEvent e) {
        ...
        if (e.getType() == TableModelEvent.INSERT) {
            ...
            try {
                ...
                if (method != null) {
                    method.invoke(entity, this.getTableModelListener().getDataTranstor(curColumn,
                            this.getValueAt(e.getFirstRow(), e.getColumn()), getMethod.getReturnType()));
                } else {
                    Field field = this.getCls().getDeclaredField(colName);
                    field.setAccessible(true);
                    field.set(entity, this.getTableModelListener().getDataTranstor(curColumn,
